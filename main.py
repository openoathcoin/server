...
if __name__ == "__main__":
  ...
  gui.run(host="localhost", port=8000, title=title, favicon=logo_file, watermark="", debug=True, use_reloader=True)  # default host:port 127.0.0.1:5000
  # gui.run(host="localhost", port=8000, title=title, favicon=logo_file, watermark="")  # run in notebook
else:
  # for production
  # run source env/bin/activate to activate virtual environment
  ...
  app = gui.run(host="localhost", port=8000, title=title, favicon=logo_file, watermark="", run_server=False)
